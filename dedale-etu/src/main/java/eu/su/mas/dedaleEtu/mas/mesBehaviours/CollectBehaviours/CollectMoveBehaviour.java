 ...
 *
 */
public class CollectMoveBehaviour extends SimpleBehaviour {
 ...
 */
	public CollectMoveBehaviour(final AbstractDedaleAgent myagent) {
		super(myagent);
		//this.myMap=myMap;
	...
	public void action() {
		...
		if(this.myMap==null) {
			...
			((AgentCollect) this.myAgent).setMyMap(this.myMap);
		}
		***/
		...
		boolean exploDone = ((AgentCollect) this.myAgent).getExploDone();
		...
		String myEntityType = ((AgentCollect) this.myAgent).getMyEntityType();
		...
		if (myPosition!=null){
			...
			if (!this.myMap.hasOpenNode()){
				...
				((AgentCollect) this.myAgent).setExploDone();
				//exit = 4; // RandomWalk
				// finished = true;
			...
			exploDone = ((AgentCollect) this.myAgent).getExploDone();
			
			List<Couple<Observation, Integer>> list = ((AbstractDedaleAgent) this.myAgent).getBackPackFreeSpace();
			
			for (Couple<Observation, Integer> couple: list) {
				if (couple.getLeft().equals(myTreasureType)) {
					...
					if (isBlocked) {
						...
						while (dist < 3) { // deplacement de 3 noeuds
							...
							for (Couple<Location,List<Couple<Observation,String>>> o: list_obs)  {
								...
								List<Couple<Observation,String>> liste = o.getRight();
								
								// PrintColor.print(this.myAgent.getLocalName(), "Je regarde en " + loc.getLocationId());
								
								if ((liste.size() == 0) && (!path.contains(loc.getLocationId())) && (!foundFreeLoc) && (!loc.getLocationId().equals(((AbstractDedaleAgent)this.myAgent).getCurrentPosition().getLocationId()))) {
									nextNodeId = loc.getLocationId();
									foundFreeLoc = true;
									
								} else {
									for (Couple<Observation,String> c: liste) {
			...
			for (String a : ((AgentCollect) this.myAgent).getMyDico().keySet()) {
				PrintColor.print(this.myAgent.getLocalName(), "Agent : " + a);
				PrintColor.print(this.myAgent.getLocalName(), ((AgentCollect) this.myAgent).getMyDico().get(a).getSerializableGraph().toString());
			}
			PrintColor.print(this.myAgent.getLocalName(), "***");
			***/
	...
	public List<String> getTreasuresPos (String myEntityType, Location myPosition) {
		...
		Observation myTreasureType = ((AbstractDedaleAgent) this.myAgent).getMyTreasureType();
		List<String> treasures_pos = ((AgentCollect) this.myAgent).getToVisit(myTreasureType); // liste de tous les trésors à parcourir
		...
		if (treasures_pos.contains(myPosition.getLocationId())) { // si je suis sur le trésor
			((AgentCollect) this.myAgent).addVisitedPos(myPosition.getLocationId());
			treasures_pos.remove(myPosition.getLocationId());	
			
			if (treasures_pos.size() == 0) {				
				treasures_pos = ((AgentCollect) this.myAgent).getToVisit(myTreasureType); // liste de tous les trésors à parcourir
			}
