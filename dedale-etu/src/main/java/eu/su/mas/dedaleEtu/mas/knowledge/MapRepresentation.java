...
public class MapRepresentation implements Serializable {
	...
	public MapRepresentation(boolean display) {
		...
		if (!display) {
			...
			System.out.println("Erreur d'initialisation de MapRepresentation()");
		}
	}
	...
	public List<String> getShortestPathToClosestObject(String myPosition, List<String> data) {
		System.out.println("objects : " + data);
		
		//1) select the closest one
		List<Couple<String,Integer>> lc=
				data.stream()
				.map(on -> (getShortestPath(myPosition,on)!=null)? new Couple<String, Integer>(on,getShortestPath(myPosition,on).size()): new Couple<String, Integer>(on,Integer.MAX_VALUE))//some nodes my be unreachable if the agents do not share at least one common node.
				.collect(Collectors.toList());
	...
	public void mergeMap(SerializableSimpleGraph<String, MapAttribute> sgreceived) {
		...
		for (SerializableNode<String, MapAttribute> n: sgreceived.getAllNodes()){
			...
			if (!alreadyIn) {
				...
				// en rose juste pour le style, sans perdre l'info d'origine
	...
	/***
	public boolean equalsGraph(SerializableSimpleGraph<String, MapAttribute> otherGraph) {
	    //if (otherGraph == null) return false;

	    // Convertir le graphe actuel en un SerializableSimpleGraph pour comparaison
	    SerializableSimpleGraph<String, MapAttribute> thisGraph = this.getSerializableGraph();

	    // Vérifier que le nombre de nœuds est le même
	    if (thisGraph.getAllNodes().size() != otherGraph.getAllNodes().size()) {
	    	System.out.println("\n====== NB NODES différents\n");
	        return false;
	    }

	    // Vérifier que chaque nœud a le même attribut
	    for (SerializableNode<String, MapAttribute> node : thisGraph.getAllNodes()) {
	        SerializableNode<String, MapAttribute> otherNode = otherGraph.getNode(node.getNodeId());
	        if (otherNode == null || !node.getNodeContent().equals(otherNode.getNodeContent())) {
	        	System.out.println("\n====== ATTRIBUTS DU NOEUD différents\n");
	            return false;
	        }
	    }

	    // Vérifier que chaque arête existe dans les deux graphes
	    for (SerializableNode<String, MapAttribute> node : thisGraph.getAllNodes()) {
	        Set<String> thisEdges = thisGraph.getEdges(node.getNodeId());
	        Set<String> otherEdges = otherGraph.getEdges(node.getNodeId());
	    ...
	    return true; // Les graphes sont identiques
	}
